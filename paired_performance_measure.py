...
def get_f1(result_str, category=None):
    if category:
        if category not in result_str:
            ...
            return float(result_str.split(category)[1].split('      ')[3])
    else:
        return float(result_str.split('weighted avg')[1].split('      ')[3])
...
def trainer(train_data, train_labels, test_data, test_labels, weighted_f1s_categories, tag_id_maps, num_epoch=10, batch_size=32, model_name="dslim/bert-base-NER", evaluate_only=False):
    ...
    if evaluate_only:
        ...
        model.to(device)
        model.eval()
        weighted_f1, all_reports = evaluate(test_dataset, device, model, id2tag, weighted_f1s_categories)
        return weighted_f1, all_reports
...
if __name__ == '__main__':
    ...
    results = {}
    # all_reports = []
    # using Cartesian product to get all combinations with repetition
    for p_names in itertools.product(names, repeat=2):
        # getting the source data name and the target data name
        # we assume the first one is the source domain and the latter is target domain
        s_name, t_name = p_names[0], p_names[1]
        ...
        if s_name != t_name:
            s_train_data, s_train_labels, s_test_data, s_test_labels = load_data(data_folder, s_name)
            t_train_data, t_train_labels, t_test_data, t_test_labels = load_data(data_folder, t_name)

            tag2id, id2tag = get_tag2id(s_train_labels + t_test_labels)
            # test_tag2id, test_id2tag = get_tag2id(t_test_labels)

            if DEBUG:
                s_train_data, s_train_labels, s_test_data, s_test_labels = s_train_data[:10], s_train_labels[:10], s_test_data[:10], s_test_labels[:10]
                t_train_data, t_train_labels, t_test_data, t_test_labels = t_train_data[:10], t_train_labels[:10], t_test_data[:10], t_test_labels [:10]
            ...
            weighted_f1, result_str, weighted_f1s_categories = trainer(s_train_data, s_train_labels, t_test_data,
                                                                       ...
                                                                       (tag2id, id2tag),
                                                                       num_epoch=epoch)
            # all_reports.append((weighted_f1, result_str, weighted_f1s_categories))
        else:
            s_train_data, s_train_labels, t_test_data, t_test_labels = load_data(data_folder, s_name)
            tag2id, id2tag = get_tag2id(s_train_labels + t_test_labels)
            # test_tag2id, test_id2tag = get_tag2id(t_test_labels)
            if DEBUG:
                s_train_data, s_train_labels, t_test_data, t_test_labels = s_train_data[:10], s_train_labels[:10], t_test_data[:10], t_test_labels[:10]
            epoch = 1 if DEBUG else 10
            weighted_f1, result_str, weighted_f1s_categories = trainer(s_train_data, s_train_labels, t_test_data,
                                                                       t_test_labels, weighted_f1_categories, (tag2id, id2tag),
                                                                       num_epoch=epoch)
            # all_reports.append((weighted_f1, result_str, weighted_f1s_categories))
